...
let currentUrlId = searchParams.get("id"); //  getting current url id// Fetching book when page is loaded
window.addEventListener("DomContentLoaded", getBook());

function getBook() {
  fetch(`https://bookshop-api.mirkwood.dev/books/${currentUrlId}`)
    .then((res) => {
      return res.json();
    })
...
function displayBook(book) {
  ...
  let currentBook = "";
  let bookID = book.id; // getting id of current book
  // comparing current book's id with current  url id
